...
if (-not $createdNew) {
   ...
   # --- CORREÇÃO: Envia o retorno "OK" para o vazio, silenciando o terminal ---
   [System.Windows.Forms.MessageBox]::Show("O Clone Commander já está aberto.", "Aviso", 0, 48) | Out-Null
   exit
}
...
# ====================================================================
# --- GERADOR DE ÍCONE DINÂMICO NATIVO (MÉTODO SUPER-SAMPLING) ---
# ====================================================================
try {
    # 1. TÁTICA DE SUPER-SAMPLING: Desenhamos um ícone GIGANTE (64x64).
    # O Windows encolhe imagens grandes com perfeição em qualquer escala de DPI.
    $w = 64
    $h = 64
    ...
    # 2. LIGA OS MOTORES DE ALTA QUALIDADE FOTOGRÁFICA
    ...
    try {
        ...
        if ($sysTheme -eq 0) { $isDarkTaskbar = $true }
    } catch { }
    ...
    # 3. FONTE GIGANTE: 50 Píxeis cravados para máxima nitidez antes do encolhimento
    $fontSizePx = 50
    ...
    # O GetHicon converte o Bitmap 64x64 num Ícone puro de 64x64
    $hIcon = $bmp.GetHicon()
